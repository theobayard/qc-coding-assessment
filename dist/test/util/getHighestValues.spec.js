...
describe("getHighestValue", () => {
    it("Should not fail on an empty object", () => {
        (0, chai_1.expect)((0, getHighestValues_1.default)({}).length).to.equal(0);
    });
    it("Should work with only one element", () => {
        (0, chai_1.expect)((0, getHighestValues_1.default)({ test: 1 })[0]).to.equal("test");
    });
    it("Should work with multiple elements of equal value", () => {
        ...
        for (let numEqualElements = 1; numEqualElements < numElementsToAdd; numEqualElements++) {
            ...
            const highestValues = (0, getHighestValues_1.default)(testingObject);
            (0, chai_1.expect)(highestValues.length).to.equal(numEqualElements);
        }
    });
    it("Should only return the highest value", () => {
        // Function that guarantees it found find and only find as the key
        // with the highest value
        const findsThisHighest = (find, inThis) => {
            const highestValues = (0, getHighestValues_1.default)(inThis);
            (0, chai_1.expect)(highestValues[0]).to.equal(find);
            (0, chai_1.expect)(highestValues.length).to.equal(1);
        };
        let testingObject = {
            test1: 1,
            test2: 2
